...
class attendeeController {
    ...
    async register(req, res) {
        try {
            ...
            if (attendeeexist) {
                if (attendeeexist.isdelete == true) {
                    const id = attendeeexist._id
                    const data = { first_name: first_name, last_name: last_name, email: email, phone: phone, dob: dob, gender: gender, password: hashedpassword, isThirteenPlus: isThirteenPlus }
                    const addattendee = await attendeeRepositories.findbyid(data, id);
                    if (addattendee) {
                        return res.status(errorCode.success).json({ message: 'attendee registered successfully' });
                    }
                }
   ...
   async otpverify(req, res) {
    ...
    async testimonial(req, res) {
        try {
            ...
            const data = { commentdata, rating, userid: user._id }
